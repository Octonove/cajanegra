# Construye el ejecutable de CajaNegra con PyInstaller (onedir).
...
Push-Location $root
& $py -m PyInstaller --noconfirm --clean (Join-Path $PSScriptRoot "CajaNegra.spec")
$code = $LASTEXITCODE
Pop-Location

if ($code -eq 0) {
    Write-Host "`n== LISTO ==" -ForegroundColor Green
    Write-Host "Ejecutable: $(Join-Path $root 'dist\CajaNegra\CajaNegra.exe')"
} else {
